    ...
    def choice_channel(self, idx):
        ...
        # 我的歌单
            ...
            if self.userid is None:
                # 使用本地存储了账户登录
                if self.account:
                    login_info = netease.login(self.account[0], self.account[1])
                    
                # 本地没有存储账户，或本地账户失效，则引导录入
                if self.account == [] or login_info['code'] != 200:
                ...
                self.username = login_info['profile']['nickname']
                self.userid = login_info['account']['id']
            # 读取登录之后的用户歌单
